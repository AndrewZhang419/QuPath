...
colors = [-377282,
          -9408287]  # their associated color, see selection of different color values at the bottom of the file
...
os.environ['PATH'] = 'C:\\research\\openslide\\bin' + ';' + os.environ[
...
for y in tqdm(range(0, osh.level_dimensions[0][1], round(tilesize * scalefactor)), desc="outer", leave=False):
    for x in tqdm(range(0, osh.level_dimensions[0][0], round(tilesize * scalefactor)), desc=f"inner {y}", leave=False):
        ...
        if mask_patches:
            ...
            exteriors_shifted = [(ext - np.asarray([(x - paddingsize), (y - paddingsize)])) // scalefactor for ext in
                                 exteriors]
        ...
        for hit, arr, id in zip(hits, arr_out, id_out):
            px, py = hit.coords[:][
                0]  # this way is faster than using hit.x and hit.y, likely because of call stack overhead
        ...
        for id, classid in zip(id_out, classids):
            allobjects[int(id)]["properties"]['classification'] = {'name': classnames[classid],
