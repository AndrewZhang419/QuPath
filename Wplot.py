...
def load_qupath_objects(json_path):
    if json_path.endswith('.gz'):
        with gzip.open(json_path,'rt') as f: return geojson.load(f)
    with open(json_path,'r') as f:            return geojson.load(f)
...
def analyze_json(json_path, wsi_path, level=0, tilesize=10000):
    ...
    dfc = df.groupby(['tx','ty','Class'], sort=False) \
            .size() \
            .reset_index(name='Count')
    dfc['Annotation'] = os.path.splitext(os.path.basename(json_path))[0]
    return dfc
...
def main():
    ...
    gp.add_argument('-j','--json',     dest='json_path',
                    help='One QuPath JSON file')
    gp.add_argument('-J','--json-dir', dest='json_dir',
                    help='Folder of QuPath JSONs (*.json)')
    p.add_argument('-w','--wsi', dest='wsi_path', required=True,
                   help='Whole‐slide image file (.svs)')
    p.add_argument('--level',   type=int, default=0,
                   help='OpenSlide level to use')
    p.add_argument('--tilesize',type=int, default=10000,
    ...
    med = df.groupby(['Annotation','Class'])['Log2Count'].median().unstack()
    xcoords = np.arange(len(files))
    ax.plot(xcoords, med['Negative'], '--', color='black', linewidth=1)
    ax.plot(xcoords, med['Positive'], '--', color='black', linewidth=1)
    ...
    ax.legend(handles[:2], labels[:2], title='Class', loc='upper right')
    ax.set_ylabel('log₂(Cells per Tile + 1)')
    plt.title('LINE1–ORF1 High and Low Tumor Cells')
